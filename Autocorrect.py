...
def switch_letter(word, verbose=False):
    ...
    ### START CODE HERE ###
    split_l = [(word[:i],word[i:]) for i in range(len(word))]
    switch_l = [L[:len(L)-1]+R[0]+L[len(L)-1]+R[1:] for L,R in split_l if L]
...
def replace_letter(word, verbose=False):
    ...
    ### START CODE HERE ###
    split_l = [(word[:i],word[i:]) for i in range(len(word))]
    replace_l=[L+letters[i]+R[1:] for i in range(len(letters)) for L,R in split_l if letters[i]!=R[0]]
...
def edit_one_letter(word, allow_switches = True):
    ...
    """
    edit_one = insert_letter(word) + delete_letter(word) + replace_letter(word) 
    if allow_switches:
        edit_one = edit_one + switch_letter(word) 
    edit_one_set = set(edit_one)
...
def edit_two_letters(word, allow_switches = True):
    ...
    '''
    edit_one_f = edit_one_letter(word, allow_switches)
    edit_two_f = set()
    for w in edit_one_f:
        temp = edit_one_letter(w, allow_switches)
        edit_two_f = edit_two_f.union(temp)
    
    edit_two_set = set(edit_two_f)
...
def get_corrections(word, probs, vocab, n=2, verbose = False):
    ...
    if suggestions:
        for suggestion in suggestions:
            val1[suggestion] = probs[suggestion]
    
    #Step 3: Get all your best words and return the most probable top n_suggested words as n_best
    
    n_best = None
    n_besti = sorted(val1.items(), key=lambda x:x[1], reverse=True)
    n_best = n_besti[:n]
